...
export function findBestRoute(tokenA, tokenB) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            // Get the best route from the routes module
            const bestRoute = yield routes.getBestRoute(tokenA, tokenB);
            if (!bestRoute) {
...
export function calculatePriceImpact(amountIn, amountOut, midPrice, tokenInDecimals, tokenOutDecimals) {
...
export function applySlippage(amount, slippagePercentage, increase = false) {
...
export function getSwapQuote(tokenIn_1, tokenOut_1, amountIn_1) {
    return __awaiter(this, arguments, void 0, function* (tokenIn, tokenOut, amountIn, slippagePercentage = 0.5) {
        try {
            // Find the best route for the token pair
            const route = yield findBestRoute(tokenIn, tokenOut);
            ...
            const [tokenInDecimals, tokenOutDecimals, tokenInSymbol, tokenOutSymbol] = yield Promise.all([
            ...
            if (route.type === 'direct') {
                ...
                const result1 = yield provider.call({
                    to: CONTRACTS.QuoterV2,
                    data: calldata1,
                });
                ...
                const result2 = yield provider.call({
                    to: CONTRACTS.QuoterV2,
                    data: calldata2,
                });
            ...
            // Calculate price impact
            const priceImpact = calculatePriceImpact(amountIn, formattedAmountOut, midPrice, tokenInDecimals, tokenOutDecimals);
            return {
...
// Swap token to token
export function swapExactTokensForTokens(privateKey_1, tokenIn_1, tokenOut_1, amountIn_1) {
    return __awaiter(this, arguments, void 0, function* (privateKey, tokenIn, tokenOut, amountIn, slippagePercentage = 0.5 // Default 0.5% slippage
    ) {
        try {
            const provider = blockchain.getProvider();
            ...
            const quote = yield getSwapQuote(tokenIn, tokenOut, amountIn, slippagePercentage);
            ...
            const approveTx = yield tokenContract.approve(CONTRACTS.SwapRouter, amountInWei);
            ...
            if (route.type === 'direct') {
                ...
                tx = yield swapRouter.exactInputSingle(swapParams);
            }
            else {
                // Indirect route (multi-hop)
                const intermediaryToken = route.intermediaryToken;
                ...
                tx = yield swapRouter.exactInput(swapParams);
            }
...
// Swap EDU to token
export function swapExactEDUForTokens(privateKey_1, tokenOut_1, amountIn_1) {
    return __awaiter(this, arguments, void 0, function* (privateKey, tokenOut, amountIn, slippagePercentage = 0.5 // Default 0.5% slippage
    ) {
        try {
            const provider = blockchain.getProvider();
            const wallet = new ethers.Wallet(privateKey, provider);
            const fromAddress = wallet.address;
            // Get quote and route information
            const quote = yield getSwapQuote(CONTRACTS.WETH9, tokenOut, amountIn, slippagePercentage);
            const route = quote.route;
...
// Wrap EDU to WEDU
export function wrapEDU(privateKey, amount) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const provider = blockchain.getProvider();
            const wallet = new ethers.Wallet(privateKey, provider);
            ...
            const tx = yield wethContract.deposit({ value: amountInWei });
            const receipt = yield tx.wait();
            if (!receipt) {
                throw new Error('Transaction failed');
            }
...
// Unwrap WEDU to EDU
export function unwrapWEDU(privateKey, amount) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const provider = blockchain.getProvider();
            const wallet = new ethers.Wallet(privateKey, provider);
            ...
            const wethContract = new ethers.Contract(CONTRACTS.WETH9, wethAbi, wallet);
            // Check balance
            const balance = yield wethContract.balanceOf(fromAddress);
            ...
            const tx = yield wethContract.withdraw(amountInWei);
            const receipt = yield tx.wait();
            if (!receipt) {
                throw new Error('Transaction failed');
            }
...
// Swap token to EDU
export function swapExactTokensForEDU(privateKey_1, tokenIn_1, amountIn_1) {
    return __awaiter(this, arguments, void 0, function* (privateKey, tokenIn, amountIn, slippagePercentage = 0.5, // Default 0.5% slippage
    fee) {
        try {
            const provider = blockchain.getProvider();
            const wallet = new ethers.Wallet(privateKey, provider);
            ...
            const route = yield findBestRoute(tokenIn, CONTRACTS.WETH9);
            const fee = parseInt(route.path[0].feeTier);
            // Get token details
            const tokenInContract = new ethers.Contract(tokenIn, ERC20_ABI, provider);
            const tokenInDecimals = yield tokenInContract.decimals();
            const tokenInSymbol = yield tokenInContract.symbol();
            // Convert amount to token units
            const amountInWei = ethers.parseUnits(amountIn, tokenInDecimals);
            // Approve router to spend tokens
            const approveTx = yield tokenInContract.approve(CONTRACTS.SwapRouter, amountInWei);
            yield approveTx.wait();
            ...
            const tx = yield swapRouter.exactInputSingle(swapParams);
            // Unwrap WETH to ETH and send to user
            yield swapRouter.unwrapWETH9(minAmountOut, fromAddress);
            const receipt = yield tx.wait();
            if (!receipt) {
                throw new Error('Transaction failed');
            }
